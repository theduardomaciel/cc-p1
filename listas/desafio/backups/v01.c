...
void player_action(player *player, int forest[forest_size][forest_size], int monsters_amount, monster monsters[], int *defeated_monsters_amount)
{
    ...
    if (forest[player->line][player->column] != -1)
    {
        ...
        for (int i = 0; i < monsters_amount; i++)
        {
            ...
            {
                apply_buff(player, monsters[i]); // aplicamos a bonificação do jogador

                // Enquanto o jogador e o monstro estiverem vivos, eles irão lutar
                while (player->life > 0 && monsters[i].life > 0)
                {
                    printf("Monstro: %d (-%d) = %d\n", monsters[i].life, player->damage, monsters[i].life - player->damage);

                    monsters[i].life -= player->damage; // o monstro perde a vida que a qual o jogador deu dano
                    ...
                    if (monsters[i].life <= 0)
                    {
                        ...
                        *defeated_monsters_amount += 1; // incrementamos a quantidade de monstros derrotados
                        ...
                        printf("Experiência do jogador: %d (+%d) = %d\n", player->experience, monsters[i].experience_on_death, player->experience + monsters[i].experience_on_death);
                        player->experience += monsters[i].experience_on_death; // o jogador ganha a experiência do monstro
                    ...
                    else if (player->life <= 0)
                    {
                        ...
                        monsters[i].life = monsters[i].initial.life; // o monstro volta a ter a vida completa

                        break;
                    }

                    printf("Jogador: %d (-%d) = %d\n", player->life, monsters[i].damage, player->life - monsters[i].damage);

                    player->life -= monsters[i].damage; // o jogador perde a vida que o monstro deu dano
                }
...
int main()
{
    ...
    for (int round = 0; round < rounds_amount; round++)
    {
        ...
        int is_invalid = move_player(movements[round][0], &pedro, forest_size, forest, tulio_pos);
        if (is_invalid)
        {
            return;
        }

        player_action(, &pedro, tulio_pos, forest_size, forest, monsters_amount, monsters, &defeated_monsters_amount_pedro);
        forest_map(forest_size, forest, pedro, tulio);

        // Em seguida, fazemos a lógica de Túlio
        int pedro_pos[2] = {pedro.line, pedro.column};
        printf("\nTúlio:\n");
        player_action(movements[round][1], &tulio, pedro_pos, forest_size, forest, monsters_amount, monsters, &defeated_monsters_amount_tulio);
        forest_map(forest_size, forest, pedro, tulio);
    }

    printf("%d %d %d %d\n", pedro.life, pedro.initial.life, pedro.experience, defeated_monsters_amount_pedro);
    printf("%d %d %d %d\n", tulio.life, pedro.initial.life, tulio.experience, defeated_monsters_amount_tulio);
